...
if (!validateDimensions(actualDims, expectedDims, embedder.modelInfo())) {
  process.exit(1);
}
...
await runStartupReindex(vault, chroma, embedder, vaultIndex, batchLimits, !!(process.env.ENGRAM_RE_EMBED_MD));

// ── Body hash registry ──────────────────────────────────────────────────────────

const bodyHashRegistry = new BodyHashRegistry(vault.root);
bodyHashRegistry.load();
populateBodyHashRegistry(vault, bodyHashRegistry);
